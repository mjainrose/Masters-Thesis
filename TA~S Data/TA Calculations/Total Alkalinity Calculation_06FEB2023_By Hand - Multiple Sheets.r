...
# Set your working directory to the directory that contains the TALK data file on your computer

# List all files in the working directory

list.files()

# Enter your TALK data file name: use the output from list files function to copy and paste the FULL file name into the prompt
...
# Identify each sheet in the file

sheet_names <- excel_sheets(filename)



### Create loop to calculate Total Alkalinity for each sheet ###

for (i in seq_along(sheet_names)) {
  # Index and read individual TALK data sheet
  # readxl package must be installed
  
  data <- read_excel(filename, sheet = sheet_names[i])
  ...
  # BATCH A25
  dHCl <-
    1.02887 - ((1.108 * 10 ^ -4) * TempLabC) - ((4.00 * 10 ^ -6) * (TempLabC ^
                                                                      2))
  ...
  F1 <-
    (m0 + mass_HCl) * exp(E / ((8.314472 * TempK) / 96485.3399))
  ...
  E0_EST <-
    E - ((8.314472 * TempK) / 96485.3399) * log(((-m0 * TA_EST) + (mass_HCl * Conc)) / (m0 + mass_HCl))
  ...
  KS <-
    exp((-4276.1 / TempK) + 141.328 - (23.093 * log(TempK))  + (((-13856 / TempK) + 324.57 - (
      47.986 * log(TempK)
    )) * (I_mo ^ 0.5))  +  (((35474 / TempK) - 771.54 + (
      114.723 * log(TempK)
    )) * I_mo)  - ((2698 / TempK) * (I_mo ^ 1.5))  +  ((1776 / TempK) * (I_mo ^
                                                                           2))  +  log(1 - (0.001005 * Salinity)))
  ...
  Sumsq_Calc <-
    function(TA_EST,
             ...
             Conc) {
      Del <-
        TA_EST + ((ST) / (1 + ((KS * Z) / (f_EST * H)))) + ((FT) / (1 + (KF / (f_EST * H)))) + (((m0 + mass_HCl) / m0) * ((f_EST * H) / Z)) - ((mass_HCl / m0) * Conc)
  ...
  fn <-
    function(x0)
      Sumsq_Calc(x0[1], x0[2], ST, KS, H, Z, FT, KF, m0, mass_HCl, Conc)
  ...
  E0 <-
    mean(E0_mean + (((8.314472 * TempK) / 96485.3399) * log(f)))
  ...
  # Creates a new variable for the data file with results
  
  results <- data
  
  #Renames the output file
  
  out_file <- paste0(sheet_names[i], "_processed.csv")
